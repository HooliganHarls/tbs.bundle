...
def VideoMainMenu():
    ...
    showMap = dict()
    shownum=0
    for thingies in content1.xpath('//ul[@id="shows_subnav"]/li/a[@class="topnav yesquery"]'):
          shownum =shownum+1
          showID=shownum
          stuff=XML.StringFromElement(thingies)
          Log(stuff)
          title=thingies.text
          showID=re.compile('cid=([0-9]+)').findall(stuff)
          #showID=thingies.text
          Log(showID)
          titleUrl="http://www.tbs.com/video/navigation/getCollectionById/?cid="+showID[0]
          Log(titleUrl)
          Log(title)
          thumb=TBSthumb
       #  Log(thumb)
